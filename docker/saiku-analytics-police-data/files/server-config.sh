...
curl -s -u admin:admin -F name="Police_Data" -F file="@/tmp/PoliceDataMondrianSchema.xml;filename=PoliceDataMondrianSchema.xml;type=text/xml" http://localhost/saiku/rest/saiku/admin/schema/Police_Data/

echo "Installing data sources..."

curl -s -X POST -u admin:admin --header "Content-Type: application/json" -T /tmp/mondrian-connections.json http://localhost/saiku/rest/saiku/admin/datasources
